...
def rsync_run_simple(excludes:list, src, dest):
    ...
    if os.path.exists(src):
        ...
        # Get some stats before running
        print('Capturing Stats')
        rsyncStatCMD = ['rsync', '-rh', '--update', '--safe-links', '--dry-run', '--stats']
        for exclude in excludes:
            rsyncStatCMD.append('--exclude=' + exclude)
        rsyncStatCMD.append(src)
